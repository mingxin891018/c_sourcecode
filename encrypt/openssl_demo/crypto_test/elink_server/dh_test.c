...
static const unsigned char m_dh_p[] = { 0xD0, 0x72, 0x25, 0x5B, 0xCB, 0x1C, 0x0D, 0xF7, 0x73, 0xB2, 0x03, 0x0C, 0x01, 0x38, 0x34, 0x0F };
static const unsigned char m_dh_g[] = {0x05};
...
//对方公钥
static unsigned char m_pubkey[128] = {0};
static int m_pubkey_len = 0;

//自己公钥
static unsigned char *m_pubkey2 = NULL;
static int m_pubkey_len2 = 0;

int get_send_pubkey(unsigned char **pubkey, int *pubkey_len)
{
	static unsigned char base64_pubkey[64] = {0};
	*pubkey = m_pubkey2;
	*pubkey_len = m_pubkey_len2;
	int ret = base64_encode(m_pubkey2, m_pubkey_len2, base64_pubkey);

	log_info("send_pubkey:\n");
	bin_print(m_pubkey2, m_pubkey_len2);

	*pubkey = base64_pubkey;
	*pubkey_len = ret;
	log_info("send_pubkey_base64:%s\n", base64_pubkey);

}

int get_share_key(unsigned char *pubkey, int pubkey_len)
{
	int pubkey_len2 = 0, ret = -1, sharekey_len = 0;
	int sharekey_len2 = 0;
	unsigned char *pubkey2 = NULL, *sharekey = NULL;

	//初始化dh2
	DH *dh2 = dh_init_ex(m_dh_p, sizeof(m_dh_p), m_dh_g, sizeof(m_dh_g));
	if(!dh2)
	{
	...
	//生成pubkey2
	ret =dh_get_pubkey(dh2, &m_pubkey2, &m_pubkey_len2, NULL, NULL);
	if(ret < 0)
	{
	...
	//生成sharekey2
	dh_get_sharekey(dh2, m_pubkey, m_pubkey_len, &sharekey, &sharekey_len);
	if(sharekey == NULL)
	{
	...
	ret = 0;
	bin_print(sharekey, sharekey_len);
...
int analysis_register_data(unsigned char *buf, int buf_length, unsigned char *key, unsigned char *iv)
{
	int ret = 0;
...
int set_pubkey(unsigned char *pubkey, int pubkey_len)
{
	memset(m_pubkey, 0, sizeof(m_pubkey));
	int ret = base64_decode(pubkey, pubkey_len, m_pubkey);
	m_pubkey_len = ret;
	log_info("set_pubkey:\n");
	bin_print(m_pubkey, m_pubkey_len);

	get_share_key(m_pubkey, ret);
}
