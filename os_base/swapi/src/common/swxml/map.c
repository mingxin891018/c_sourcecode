#include "swapi.h"
#include "swxml.h"
#include "map.h"
#include "membuf.h"
#include "parser.h"
#include "swmem.h"

/*================================================================
*   NamedNodeMap_getItemNumber
*       return the item number of a item in NamedNodeMap.
*       Internal to parser only.
*   Parameters:
*       name: the name of the item to find
*   
*=================================================================*/
unsigned long
xml_nodemap_getItemNumber( XML_NamedNodeMap * nnMap,
                                char *name )
{
	XML_Node *tempNode;
	unsigned long returnItemNo = 0;

	assert( nnMap != NULL && name != NULL );
	if( ( nnMap == NULL ) || ( name == NULL ) ) {
		return XML_INVALID_ITEM_NUMBER;
	}

	tempNode = nnMap->nodeItem;
	while( tempNode != NULL ) {
		if( strcmp( name, tempNode->nodeName ) == 0 ) {
			return returnItemNo;
		}

		tempNode = tempNode->nextSibling;
		returnItemNo++;
	}

	return XML_INVALID_ITEM_NUMBER;
}

/*================================================================
*   NamedNodeMap_getNamedItem
*       Retrieves a node specified by name.
*       External function.
*
*   Parameter:
*       name: type nodeName of a node to retrieve.
*
*   Return Value:
*       A Node with the specified nodeName, or null if it
*       does not identify any node in this map.
*
*=================================================================*/
XML_Node *
xml_nodemap_getNamedItem( XML_NamedNodeMap * nnMap,
                               char *name )
{
	long index;

	if( ( nnMap == NULL ) || ( name == NULL ) ) {
		return NULL;
	}

	index = xml_nodemap_getItemNumber( nnMap, name );
	if( index == XML_INVALID_ITEM_NUMBER ) {
		return NULL;
	} else {
		return ( xml_nodemap_getItem( nnMap, ( unsigned long )index ) );
	}
}

/*================================================================
*   NamedNodeMap_item
*       Returns the indexth item in the map. If index is greater than or
*       equal to the number of nodes in this map, this returns null.
*       External function.
*
*   Parameter:
*       index: index into this map.
*
*   Return Value:
*       The node at the indexth position in the map, or null if that is
*       not a valid index.
*
*=================================================================*/
XML_Node *
xml_nodemap_getItem( XML_NamedNodeMap * nnMap,
                       unsigned long index )
{
	XML_Node *tempNode;
	unsigned int i;

	if( nnMap == NULL ) {
		return NULL;
	}

	if( index > xml_nodemap_getSize( nnMap ) - 1 ) {
		return NULL;
	}

	tempNode = nnMap->nodeItem;
	for( i = 0; i < index && tempNode != NULL; ++i ) {
		tempNode = tempNode->nextSibling;
	}

	return tempNode;
}

/*================================================================
*   NamedNodeMap_getLength	
*       Return the number of Nodes in this map.       
*       External function.
*   
*   Parameters:
*
*=================================================================*/
unsigned long
xml_nodemap_getSize( XML_NamedNodeMap * nnMap )
{
	XML_Node *tempNode;
	unsigned long length = 0;

	if( nnMap != NULL ) {
		tempNode = nnMap->nodeItem;
		for( length = 0; tempNode != NULL; ++length ) {
			tempNode = tempNode->nextSibling;
		}
	}
	return length;
}

/*================================================================
*   xml_nodemap_free
*       frees a NamedNodeMap.
*       External function.
*
*=================================================================*/
void
xml_nodemap_free( XML_NamedNodeMap * nnMap )
{
	XML_NamedNodeMap *pNext;

	while( nnMap != NULL ) {
		pNext = nnMap->next;
		free( nnMap );
		nnMap = pNext;
	}
}

/*================================================================
*   NamedNodeMap_addToNamedNodeMap
*       add a node to a NamedNodeMap.
*       Internal to parser only.
*   Parameters:
*       add: the node to add into NamedNodeMap.
*   Return:
*       XML_SUCCESS or failure.
*
*=================================================================*/
int
xml_nodemap_addItem( XML_NamedNodeMap ** nnMap,
                                    XML_Node * add )
{
	XML_NamedNodeMap *traverse = NULL,
									 *p = NULL;
	XML_NamedNodeMap *newItem = NULL;

	if( add == NULL ) {
		return XML_SUCCESS;
	}

	if( *nnMap == NULL )        // nodelist is empty
	{
		*nnMap =
			( XML_NamedNodeMap * ) malloc( sizeof( XML_NamedNodeMap ) );
		if( *nnMap == NULL ) {
			return XML_INSUFFICIENT_MEMORY;
		}
		
			memset( nnMap, 0, sizeof( XML_NamedNodeMap ) );
	}

	if( ( *nnMap )->nodeItem == NULL ) {
		( *nnMap )->nodeItem = add;
	} else {
		traverse = *nnMap;
		p = traverse;
		while( traverse != NULL ) {
			p = traverse;
			traverse = traverse->next;
		}

		newItem =
			( XML_NamedNodeMap * ) malloc( sizeof( XML_NamedNodeMap ) );
		if( newItem == NULL ) {
			return XML_INSUFFICIENT_MEMORY;
		}
		p->next = newItem;
		newItem->nodeItem = add;
		newItem->next = NULL;
	}

	return XML_SUCCESS;
}
